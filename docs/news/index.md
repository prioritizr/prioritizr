...
- New
  ...
  functions for building multi- objective conservation planning
  problems. Additionally, new
  ...
  functions can be used to generate multiple combinations of trade-offs
  parameters.
- New
  ...
  functions that can be used to add penalties and constraints for costs.
...
- Update
  ...
  so that if boundary data are automatically generated by this function
  (per the default behavior, with `data = NULL`), then the boundary data
  are automatically rescaled to help avoid numerical instability issues
  (via
...
- The
  ...
  `add_phylo_div_objective()`, and `add_phylo_end_objective()` functions
  have been updated to ensure consistent optimization behavior with the
  other objectives. Although these objectives previously had additional
  terms that would help minimize costs in the case of multiple optimal
  solutions, such terms have now been removed to help improve run time.
  If you still want to minimize costs when using these objectives, this
  can be done with the hierarchical multi-objective optimization
  approach and the
  [`add_cost_penalties()`](https://prioritizr.net/reference/add_cost_penalties.md)
  function.
- The
...
- Update
  [`add_highs_solver()`](https://prioritizr.net/reference/add_highs_solver.md)
  function to support a starting solution. Note that this currently
...
- Fix [`print()`](https://rdrr.io/r/base/print.html) and
  [`summary()`](https://rspatial.github.io/terra/reference/summary.html)
  functions for
  [`problem()`](https://prioritizr.net/reference/problem.md) objects not
  showing default settings in gray.
