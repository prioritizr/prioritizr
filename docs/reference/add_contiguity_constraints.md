...
#> Error in category_layer(solve(p7)): ℹ In argument to `x`.
#> Caused by error:
#> ! Error 10009: HostID mismatch (licensed to 2890c3bc, hostid is d03f011a)

# plot solutions
plot(s7, main = "solution", axes = FALSE)
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'plot': object 's7' not found
# }
```
