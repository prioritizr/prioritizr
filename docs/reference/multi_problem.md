...
- ...:
  ...
  variable types. They may have different cost and feature data, as well
  as objectives, targets, constraints, and penalties.
...
weights, and penalties. Additionally, they may also have different cost
data and features. Note that any constraint specified in one of the
[`problem()`](https://prioritizr.net/reference/problem.md) objects will
be applied during all stages of multi-objective optimization. For
example, this means that if one of the
[`problem()`](https://prioritizr.net/reference/problem.md) objects has
locked in constraints (per
[`add_locked_in_constraints()`](https://prioritizr.net/reference/add_locked_in_constraints.md)),
then these constraints will be applied during all stages of
multi-objective optimization. As such, we recommended adding constraints
to only one of the
[`problem()`](https://prioritizr.net/reference/problem.md) objects to
reduce processing time. Additionally, since budgets specified in
budget-limited objectives (e.g.,
...
are (effectively) treated as constraints, they will also be applied
during all stages of multi-objective optimization.
