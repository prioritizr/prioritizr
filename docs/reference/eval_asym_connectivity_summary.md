...
Specifically, it is calculated as the sum of the connectivity values
(per `data`) that correspond pairs of planning units, wherein one
planning unit is selected by the solution and the other planning unit is
not selected by solution.
