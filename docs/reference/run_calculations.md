...
different versions of the same conservation planning problem that
involve the same pre-processing steps (e.g., calculating boundary data),
because means that the same calculations will not be run multiple times.
...
object. As such, it does not return anything. To use this function with
[`pipe()`](https://rdrr.io/r/base/connections.html) operators, use the
