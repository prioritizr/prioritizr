...
Multi-objective optimization provides a framework to examine trade-offs and identify solutions that represent a desirable compromise among multiple objectives [@r72; @r71]. To apply multi-objective optimization, users should first identify the fundamental objectives (goals) that underpin the planning process and the trade-offs they wish to examine (e.g., connectivity, representation of different types of features, or competing human uses). 

For each fundamental objective, users should formulate a separate conservation planning problem using the `problem()` function. These individual problems can then be combined into a multi-objective problem wwith the `multi_problem()` function. Once a `multi_problem()` object has been created, it can be customized by adding (i) an approach to specify how multiple objectives should be accommodated during optimization, and (ii) a solver to specify the optimization software settings. After completing these steps, the `solve()` function can be used to generate a single or multiple solutions, that aim to achieve multiple objectives.

When constructing `multi_problem()` object, it is important to ensure that each `problem()` object has comparable planning units, decision types, and zones. For example, if using `terra::rast()` planning units, then each `problem()` should have planning units that have the same resolution, dimensionality (number of rows and columns), and coordinate reference system. Although the planning units in different `problem()` objects can have different values (e.g., one `problem()` may have costs based on human pressure and another `problem()` may have costs based on spatial area), they must have missing (`NA`) values in the same cells (or rows if planning units are `sf::st_sf()` objects). If you try building a `multi_problem()` object based on `problem()` objects that do not have comparable planning units and zones, then it will throw an error. Note that each `problem()` object can have different planning unit costs, features, constraints, objectives, and penalties.
...
* __Weighted sum__: This approach linearly combines multiple objectives using user-defined weights (`weights`), such that objectives with larger weights are intended to exert greater influence on the optimization process [@r72]. Although conceptually simple and among the most widely used approaches for multi-objective spatial optimization, the weighted sum approach is sensitive to differences in the scale of objective values [@r69]. Consequently, the influence of an objective on the optimization process may not be proportional to its assigned weight. As a result, substantial calibration is often required to identify weights that accurately reflect stakeholder preferences and motivations. Given these limitations, we generally recommend alternative approaches when possible.
```{r, fig.height = h, fig.width = w, message = FALSE, results = "hide"}
...
```
* __Hierarchical__: This approach involves solving an optimization problem for each objective in a hierarchical (lexicographic) manner, wherein those associated with a higher priority are solved before those with a lower priority [@r72]. By default, the order of priority is based on the order of problems in the `multi_problem()` object. To express trade-offs, this approach uses relative tolerance (`rel_tol`) parameters that allow the optimization process to degrade previously optimized objectives so that solutions can achieve better performance when optimizing subsequent (lower priority) objectives. This approach is especially well-suited for characterizing the full range of trade-offs between different objectives (in other words, generating a Pareto frontier).
```{r, fig.height = h, fig.width = w, message = FALSE, results = "hide"}
...
The multi-objective optimization approaches can also be used to generate multiple solutions. Although portfolio functions can also be used to generate multiple solutions, the approach functions are different because they are designed to generate solutions that have different levels of performance in achieving different objectives. Conversely, the portfolio functions are designed to generate solutions that have the (approximately) same performance and have different spatial configurations. For example, here we will use the hierarchical approach to generate a range of different solutions to explore trade-offs between representing keystone and iconic species. Additionally, the `approach_rel_tol_matrix()` and `approach_weights_matrix()` functions can be used to generate values for the `rel_tol` and `weights` parameters of approach functions.
