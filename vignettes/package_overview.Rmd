...
The _prioritizr R_ package uses a grammar to describe elements of conservation planning. This means that functions are organized into verbs that relate to specific concepts. For example, the functions used to specify the primary objective for optimizations all end with the suffix `_objective` (e.g., `add_min_set_objective()` and `add_min_shortfall_objective()`). By combining multiple functions together, they can be used to formulate a complete conservation planning problem. Specifically, the verbs for formulating problems are used to:
...
After building a conservation planning problem, it can be solved to generate a prioritization (using the `solve()` function). There are also verbs available for evaluating solutions:
...
print(p31)
```
* __*CBC* solver__: [*CBC*](https://projects.coin-or.org/Cbc) is an
...
print(p33)
```
* __*Rsymphony* solver__: This solver provides a different interface to the [_SYMPHONY_](https://projects.coin-or.org/SYMPHONY) software. It uses the _Rsymphony R_ package which is available on The Comprehensive R Archive Network (CRAN). This solver is generally slower than the other solvers, because it cannot use parallel processing.
```{r}
...
```
* __Connectivity summary__: Calculate the connectivity facilitated within a solution.
```{r}
# calculate statistic
