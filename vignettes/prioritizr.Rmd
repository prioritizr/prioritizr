...
* [evaluate](https://prioritizr.net/reference/evaluation.html) a solution to a conservation planning problem
...
After obtaining a solution to a conservation planning problem, it can be useful to calculate various summary statistics to understand its performance. The following functions are available for evaluating a solution:
...
eval_feature_representation(p37, s37)
```
...
eval_connectivity(p37, s37, data = connectivity_matrix(sim_pu_raster,
                                                       sim_features[[1]]))
```
...
Below we will generate a solution, and then calculate irreplaceability scores for the planning units selected in the solution using the three different approaches.
