...
cellStats(sim_pu_raster * s38, "sum")

# calculate how well features are represented in the solution
r38 <- feature_representation(p38, s38)
print(r38)
```
...
We recommend using replacement cost scores for small and moderate sized problems (e.g. less than 30,000 planning units) when it is feasible to do so. It can take a very long time to compute replacement cost scores, and so it is simply not feasible to compute these scores for particularly large problems. For moderate and large sized problems (e.g. more than 30,000 planning units),  we recommend using the rarity weighted richness scores. Beware, it has been known for decades that such static measures of biodiversity lead to poor conservation plans [@r17]. There is also an experimental function for calculating irreplaceability using the Ferrier method. We do not recommend using this method until it has been verified by a statistical expert.
...
fs38 <- ferrier_score(p38, s38)[["total"]]
...
rwr38 <- rarity_weighted_richness(p38, s38)
