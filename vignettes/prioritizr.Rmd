...
```{r}
# calculate statistic
eval_n_summary(p38, s38)
```
* Calculate the total cost of a solution.
```{r}
# calculate statistic
eval_cost_summary(p38, s38)
```
...
eval_feature_representation_summary(p38, s38)
```
...
eval_connectivity_summary(
  p38, s38, data = connectivity_matrix(sim_pu_raster, sim_features[[1]]))
```
...
plot(rc39, main = "replacement cost")
```

```{r, fig.height = h, fig.width = w}
# calculate Ferrier scores and extract total score
fs39 <- eval_ferrier_importance(p38, s38)[["total"]]

# plot Ferrier scores
plot(fs39, main = "Ferrier scores")
```

```{r, fig.height = h, fig.width = w}
# calculate rarity weighted richness scores
rwr39 <- eval_rare_richness_importance(p38, s39)

# plot replacement cost scores
plot(rwr39, main = "rarity weighted richness")
```
