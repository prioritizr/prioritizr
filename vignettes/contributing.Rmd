...
Documentation should be written using _roxygen2_. Software and R package names should be written in _italics_ (e.g. \emph{prioritizr}). When referring to parameter names in the documentation, they should be encapsulated by \\code tags (e.g. \code{x} in help for _problem_). When referring to the names of columns in a data.frame object, please use the format \\code{"column_name"}, where `column_name` is the name of the column (e.g. \\code{"mpg"} to refer to a column in the `mtcars` object). All help files for functions should contain at least the following sections: Description, Arguments, Value, and Examples.
