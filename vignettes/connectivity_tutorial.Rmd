...
## to ensure the prioritization covers at least 10% of the total amount
## connectivity values
s3 <- p0 %>%
      add_linear_constraints(
...
s7 <- p0 %>%
      add_boundary_penalties(penalty = 0.05, data = salt_boundary_data) %>%
      solve()
...
s7 <- p0 %>%
      add_connectivity_penalties(penalty = 0.05, data = salt_con_scores) %>%
      solve()
