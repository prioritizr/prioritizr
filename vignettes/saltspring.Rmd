...
The _prioritizr R_ package supports four different integer linear programming solver packages: _gurobi_, _cplexAPI_, _Rsymphony_, and _lpsymphony_. There are costs and benefits associated with each of these solvers, but the solver itself should have little impact on the actual solution returned (though certain solvers may take longer to return solutions than others).

First, remember that the solvers must be installed. You can check if these packages are installed by running the code below. Note that the _gurobi_ package is distributed with the [_Gurobi_ commercial software suite](https://www.gurobi.com/) and is not available on the Comprehensive R Archive Network (CRAN). Please refer to the [_Gurobi Installation Guide_](gurobi_installation.html) for more information on installing the _gurobi R_ package. Additionally, the _Rcplex_ package cannot be installed unless the [IBM CPLEX software](https://www.ibm.com/analytics/cplex-optimizer) is [already installed](https://CRAN.R-project.org/package=cplexAPI/INSTALL).
