...
eval_feature_representation_summary(p44, s44)
```
...
# calculate statistic
eval_connectivity_summary(p44, s44, data = cm)
```
...
# calculate statistic
eval_asym_connectivity_summary(p44, s44, data = acm)
```
...
# plot solution
plot(
  s45, col = c("grey90", "darkgreen"), main = "Solution",
  xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1), axes = FALSE
)
```
...
rc45 <-
  ...
  eval_replacement_importance(s45)

# plot replacement cost scores
plot(
  rc45, main = "Replacement cost scores",
  xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1), axes = FALSE
)
```
...
rs45 <-
  p45 %>%
  add_default_solver(gap = 0, verbose = FALSE) %>%
  eval_rank_importance(s45, n = 10)

# plot replacement cost scores
plot(
  rs45, main = "Rank scores",
  xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1), axes = FALSE
)
```
...
fs45 <- eval_ferrier_importance(p45, s45)[["total"]]

# plot Ferrier scores
plot(
  fs45, main = "Ferrier scores",
  xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1), axes = FALSE
)
```
...
rwr45 <- eval_rare_richness_importance(p45, s45)

# plot rarity weighted richness scores
plot(
  rwr45, main = "Rarity weighted richness scores",
  xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1), axes = FALSE
)
```
