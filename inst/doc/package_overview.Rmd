...
print(p33)
```
* __*HiGHS* solver__: #' [*HiGHS*](https://highs.dev/)) is an open source optimization software. Although formal benchmarks have yet to be completed, preliminary analyses suggest that it is faster than some of the other open-source solvers (i.e., `add_rsymphony_solver()`, `add_lpsymphony_solver()`).
```{r}
...
```{r, fig.height = h, fig.width = w}
# formulate the problem
p43 <- problem(sim_pu_raster, sim_features) %>%
       add_min_set_objective() %>%
       add_relative_targets(0.1) %>%
       add_binary_decisions()
...
# plot solution
plot(s43, col = c("grey90", "darkgreen"), main = "Solution",
     xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1))
```
...
plot(rc43, main = "Replacement cost scores")
```
...
plot(rwr43, main = "Rarity weighted richness")
```
