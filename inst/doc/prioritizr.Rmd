...
vignette: >
  %\VignetteIndexEntry{prioritizr: Systematic Conservation Prioritization in R}
...
p24 <- problem(sim_pu_raster, sim_features[[1:4]]) %>%
       add_min_set_objective() %>%
       add_relative_targets(0.1) %>%
...
Currently, the _prioritizr R_ package supports five different solvers.
...
# plot solution
plot(s39, col = c("grey90", "darkgreen"), main = "Solution",
...
```{r}
# calculate statistic
eval_n_summary(p39, s39)
```
* Calculate the total cost of a solution.
```{r}
# calculate statistic
eval_cost_summary(p39, s39)
```
...
eval_feature_representation_summary(p39, s39)
```
...
eval_connectivity_summary(
  p39, s39, data = connectivity_matrix(sim_pu_raster, sim_features[[1]]))
```
...
rc40 <- p40 %>%
        ...
        eval_replacement_importance(s39)

# plot replacement cost scores
plot(rc40, main = "replacement cost")
```

```{r, fig.height = h, fig.width = w}
# calculate Ferrier scores and extract total score
fs40 <- eval_ferrier_importance(p40, s40)[["total"]]

# plot Ferrier scores
plot(fs40, main = "Ferrier scores")
```

```{r, fig.height = h, fig.width = w}
# calculate rarity weighted richness scores
rwr40 <- eval_rare_richness_importance(p40, s40)

# plot replacement cost scores
plot(rwr40, main = "rarity weighted richness")
```
