...
## also note that we set the budget as 1e6 because this is an arbitrarily
## high budget (i.e., greater than cost of selecting all planning units)
## so the budget has no effect on the optimization
p4 <-
  problem(tas_pu, tas_features, cost_column = "cost") %>%
  add_min_penalties_objective(budget = 1e6) %>%
  add_boundary_penalties(penalty = 1, data = tas_bd) %>%
  add_relative_targets(0.17) %>%
  add_locked_in_constraints("locked_in") %>%
  add_binary_decisions() %>%
...
# solve problem
s4 <- solve(p4, force = TRUE, run_checks = FALSE)
```
