...
- Update `solve()` to throw a helpful error message if attempting to solve
  a `multi_problem()` object that does not have a specified multi-objective
  optimization approach.
- Update `solve()` to throw a helpful error message if attempting to solve
  a `multi_problem()` object that contains `problem()` objects that have
  conflicting locked constraints.
...
- New `multi_problem()` and `multi_compile()` functions for building multi-
  objective conservation planning problems. Additionally,
...
- Update `add_boundary_penalties()` so that if boundary data are automatically
  generated by this function (per the default behavior, with `data = NULL`),
  then the boundary data are automatically rescaled to help avoid
  numerical instability issues (via `rescale_matrix()`).
- The `add_max_wtd_sum_objective()`, `add_max_n_targets_met_objective()`,
  ...
  such terms have now been removed to help improve run time.
...
- Fix bug in internal `planning_unit_solution_status` function so that
  ...
  `problem()`.
- Fix `add_gurobi_solver()`, `add_gap_portfolio()`, `add_extra_portfolio()`, and
  `add_top_portfolio()` functions so that they throw an error message
  that does not have a typo when the _gurobi_ package is not available.
...
- Fix bug in `compile()` that caused unreadable error message when the same
  planning unit is locked in and locked out.
