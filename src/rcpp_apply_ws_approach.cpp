...
SEXP rcpp_apply_ws_approach(
  ...
  Rcpp::print(Rcpp::wrap("here2"));
  // specify objective values for multi-objective problem
  double curr_sign;
  for (std::size_t i = 0; i < n; ++i) {
    /// compute objective sign
    curr_sign = opt[i]->_modelsense == "min" ? 1.0 : -1.0;

    /// store values for planning unit status variables
    for (std::size_t j = 0; j < n_status; ++j) {
      mopt->_obj[j] += opt[i]->_obj[j] * curr_sign * weights[i];
    }
    /// store values for extra variables
    for (std::size_t j = n_status; j < opt_n_ncol[i]; ++j) {
      mopt->_obj[j + opt_col_offset[i]] =
        opt[i]->_obj[j] * curr_sign * weights[i];
    }
  }
  ...
  // return pointer for multi-obdjective optimization problem
