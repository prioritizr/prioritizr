...
bool rcpp_apply_asym_connectivity_penalties(SEXP x, double penalty,
   ...
   *    of elements in data.
   *
   * 2. each sparse matrix in data a sparse matrix with cells in both
   *    the upper and lower triangles and the diagonal filled in.
   *    This is important to account for asymmetric connectivity.
   *
  ...
  for (std::size_t z1 = 0; z1 < ptr->_number_of_zones; ++z1) {
    for (std::size_t z2 = 0; z2 < ptr->_number_of_zones; ++z2) {
      // scale the connectivity matrix by the penalty value
      ...
      for (arma::sp_mat::const_iterator it = curr_matrix.begin();
        ...
        if (std::abs(curr_value) > 1.0e-15) {
          if ((curr_i == curr_j) && (z1 == z2)) {
            ...
            pu_zone_penalties[curr_col1] += (mult2 * curr_value);
          } else {
            ...
            pu_b.push_back(mult2 * curr_value);
            // add the connectivity value to the weights
            pu_zone_penalties[curr_col1] += (mult1 * curr_value);
          }
        }
      }
    }
