...
#BUT: currently we lose our attributes somewhere when we are working with a matrix rel_tol input (although they are all still there in line 79 of add_hierarchical_approach() in sols)
test1 <- list(multi_s1, multi_s2, multi_s1, multi_s2, multi_s1) #PLACEHOLDER TO GET CODE WORKING

obj_mat <- matrix(NA, nrow = length(multi_s5), 
                  ncol = length(attributes(test1[[1]])$objective))#length(attributes(multi_s5[[1]])$objective))

for (i in seq_len(length(multi_s5))) {
  obj_mat[i, ] <- attributes(test1[[i]])$objective
}
