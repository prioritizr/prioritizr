...
#' 
#' The reserve design problem for the minimum set objective can be expressed mathematically 
#' for \emph{n} planning units as:
#' 
#' \deqn{\text{Minimize} \sum_{i=1}^{n}x_i c_i + \text{ subject to } \sum_{j=1}^{n}x_j 
#' r_{ij}\geq T_i \space \forall \space i}{Minimize \sum^n(xi*ci) subject to \sum^n(j*rij) 
#' \ge Ti for all i}
#' 
#' where \emph{xi} is a binary decision variable specifying whether planning unit 
...
#' unit \emph{j}, and \eqn{T_i}{Ti} is the target for feature \emph{i}. The first term 
#' is the objective function and the second is the set of constraints. In words this 
#' says 'find the set of planning units that meets all the representation targets 
