...
#' This metric is useful for evaluating the spatial fragmentation of
...
#'
#' @details
#' This metric is equivalent to the `Connectivity_Edge` metric reported by the
...
#' @name eval_boundary
#'
#' @seealso [problem()], [evaluation], [add_boundary_penalties()].
#'
#' @examples
#' \dontrun{
...
#'
#' # calculate boundary associated with the solution
#' r2 <- eval_boundary(p2, s2[, "solution_1"])
#' print(r2)
#'
...
#'
#' # calculate boundary associated with the solution
#' r3 <- eval_boundary(p3, s3[, c("solution_1_zone_1",
#'                                "solution_1_zone_2",
#'                                "solution_1_zone_3")])
#' print(r3)
#'
#' }
#' @export
eval_boundary <- function(x, ...) UseMethod("eval_boundary")

#' @rdname eval_boundary
#' @method eval_boundary default
#' @export
eval_boundary.default <- function(x, ...) {
  stop("argument to x must be a ConservationProblem object")
}

#' @rdname eval_boundary
#' @method eval_boundary ConservationProblem
#' @export
eval_boundary.ConservationProblem <- function(
  ...
  if (number_of_zones(x) == 1) {
    ...
    out <- tibble::tibble(statistic = "overall", boundary = v)
  } else {
    ...
    out <- tibble::tibble(
      statistic = c("overall", zone_names(x)), boundary = c(v, zv))
  }
  # return result
  out
}
