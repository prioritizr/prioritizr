...
#'
#' @details
#' This metric is comparable to the `Connectivity_In` metric reported by the
...
#'
#' @seealso [problem()], [evaluation], [add_connectivity_penalties()].
#'
#' @examples
#' \dontrun{
...
#'
#' }
#' @name eval_connectivity
#'
#' @exportMethod eval_connectivity
#'
#' @aliases eval_connectivity,ConservationProblem,ANY,ANY,Matrix-method eval_connectivity,ConservationProblem,ANY,ANY,matrix-method eval_connectivity,ConservationProblem,ANY,ANY,dgCMatrix-method eval_connectivity,ConservationProblem,ANY,ANY,data.frame-method eval_connectivity,ConservationProblem,ANY,ANY,array-method
NULL

#' @export
methods::setGeneric("eval_connectivity",
  signature = methods::signature("x", "solution", "zones", "data"),
  function(x, solution, zones = diag(number_of_zones(x)), data)
    standardGeneric("eval_connectivity"))

#' @name eval_connectivity
#' @usage \S4method{eval_connectivity}{ConservationProblem,ANY,ANY,matrix}(x, solution, zones, data)
#' @rdname eval_connectivity
methods::setMethod("eval_connectivity",
  methods::signature("ConservationProblem", "ANY", "ANY", "matrix"),
  function(x, solution, zones, data) {
     eval_connectivity(x, solution, zones, methods::as(data, "dgCMatrix"))
})
...
#' @name eval_connectivity
#' @usage \S4method{eval_connectivity}{ConservationProblem,ANY,ANY,data.frame}(x, solution, zones, data)
#' @rdname eval_connectivity
methods::setMethod("eval_connectivity",
...
#' @name eval_connectivity
#' @usage \S4method{eval_connectivity}{ConservationProblem,ANY,ANY,dgCMatrix}(x, solution, zones, data)
#' @rdname eval_connectivity
methods::setMethod("eval_connectivity",
  ...
  function(x, solution, zones, data) {
    ...
    internal_eval_connectivity(
      x, planning_unit_solution_status(x, solution), m, data)
})

#' @name eval_connectivity
#' @usage \S4method{eval_connectivity}{ConservationProblem,ANY,ANY,array}(x, solution, zones, data)
#' @rdname eval_connectivity
methods::setMethod("eval_connectivity",
...
internal_eval_connectivity <- function(x, solution, zone_scaled_data, data) {
  # assert valid arguments
  assertthat::assert_that(
    inherits(x, "ConservationProblem"),
    is.matrix(solution),
  ...
  if (number_of_zones(x) == 1) {
    ...
    out <- tibble::tibble(statistic = "overall", connectivity = v)
  } else {
    ...
    out <- tibble::tibble(
      statistic = c("overall", zone_names(x)), connectivity = c(v, zv))
  }
  # return result
  out
}
