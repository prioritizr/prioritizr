...
#' and `Matrix` formats, each row and column corresponds
#' to a different zone in `x`, and cell values must
#' contain binary `numeric` values (i.e., one or zero) that indicate
#' if connected planning units (as specified by `data`)
#' should be still considered connected if they are allocated to
...
#' that `zones` must be symmetric, and that if a row or column has
...
#' should differ among the features, then `zones` should
...
#' objects. In particular, `data` indicates which planning units should be
#' treated as being connected when implementing constraints to ensure that
#' features are represented in contiguous units. If different features have
...
#' for which features using a `list` of objects.
#' Defaults to `NULL` such that the connection data are calculated
...
#' Here connection data are calculated automatically
...
#' }
#'
#' \item{`data` as a `list` object}{
#' Here a `matrix`, `Matrix`, or
