...
#'
#' @param x [terra::rast()] or [sf::sf()] object with planning unit data.
#'
#' @param y [terra::rast()] object with feature data.
#'
...
#'
#' @export
rij_matrix <- function(x, y, memory = NA, idx = NULL, ...) {
  assert_required(x)
  assert_required(y)
  UseMethod("rij_matrix")
}

#' @rdname rij_matrix
#' @export
rij_matrix.SpatRaster <- function(
  x, y, memory = NA, idx = NULL, ...
) {
...
#' @rdname rij_matrix
#' @export
rij_matrix.sf <- function(
  x, y, memory = FALSE, idx = NULL, fun = "sum", ...
) {
