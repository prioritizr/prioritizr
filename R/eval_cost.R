...
#'   \describe{
#'
#'   \item{statistic}{`character` description of the cost statistic.
#'     The cost statistic associated with the `"overall"` value
#'     in this column is calculated using entire solution
#'     (including all management zones if there are multiple zones).
#'     If multiple management zones are present, then cost statistics
#'     are also provided for each zone separately. In such cases,
#'     these cost statistics are associated with values, in this column,
...
#'
#' # calculate cost of the solution
#' r3 <- eval_cost(p3, s3[, c("solution_1_zone_1",
#'                            "solution_1_zone_2",
#'                            "solution_1_zone_3")])
#' print(r3)
#' }
#' @export
eval_cost <- function(x, solution) UseMethod("eval_cost")

#' @rdname eval_cost
#' @method eval_cost default
#' @export
eval_cost.default <- function(x, solution) {
  stop("argument to x must be a ConservationProblem object")
}

#' @rdname eval_cost
#' @method eval_cost ConservationProblem
#' @export
eval_cost.ConservationProblem <- function(x, solution) {
  ...
  if (x$number_of_zones() > 1) {
    out <- tibble::tibble(
      statistic = c("overall", zone_names(x)),
      cost = c(total_cost, costs))
  } else {
    out <- tibble::tibble(statistic = "overall", cost = total_cost)
  }
  # return result
  out
}
