...
#' solutions with low connectivity between planning units. A penalty acts as an explicit 
#' tradeoff with the \code{objective} 
#' being minimized or maximized, and thereby increases the cost of 
#' the soution.  For example, when the \code{add_boundary_penalties} function is added, 
#' solution fragmentation decreases but solution 
#' cost increases. 
#'
#' @details
#' Both penalties and constraints can be used as mechanisms to increase solution connectivity, 
#' but instead of penalizing solutions with low connectivity, 
#' a constraint acts as a cutoff to make certain solutions invalid. 
#' Applying a constraint does not necessarily affect solution cost, while applying a penalty does. Below 
#' are the penalties that can be added to a \code{ConservationProblem} object. 
#' 
#' \describe{
#'
...
#' use \code{add_connectivity_penalties}.
#'
#'  }
#'
...
#' @name penalties
#'
#' @seealso \code{\link{constraints}}, \code{\link{objectives}}, \code{\link{targets}},
#'   \code{\link{problem}}
NULL
