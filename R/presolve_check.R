#' @include internal.R ConservationProblem-class.R OptimizationProblem-class.R compile.R run_presolve_check.R
NULL
...
#'
#' @param warn `logical` value indicating if a warning should be thrown if the
#' check fails? Defaults to `TRUE`.
#'
#' @details
#' This function checks for issues that are likely to result in
#' "strange" or "unrealistic" solutions.
#' Specifically, it checks if (i) all planning units are
...
presolve_check_header <- function() {
  cli::cli_fmt(cli::cli_h1("Presolve checks"))
}
