...
test_that("add_rule_targets", {
  ...
  # create problem
  p <- problem(sim_pu_raster, sim_features)
  # create problem with auto_targets()
  p1 <-
    p %>%
    add_auto_targets(method = rule_targets(0.3, wt_data, ft_data))
  # create problem with add_rule_targets()
  p2 <-
    p %>%
    add_rule_targets(0.3, wt_data, ft_data)
  # run tests
  expect_equal(
    p1$targets$output(p),
    p2$targets$output(p)
...
test_that("invalid inputs", {
  ...
  expect_tidy_error(
    p %>% add_rule_targets(NA, wt_data, ft_data),
    "number"
  )
  expect_tidy_error(
    p %>% add_rule_targets(-1.2, wt_data, ft_data),
    "0"
  )
  expect_tidy_error(
    p %>% add_rule_targets(1.2, wt_data, ft_data),
  ...
  expect_tidy_error(
    p %>% add_rule_targets(NA_real_, wt_data, ft_data),
    "missing"
  )
  ## rules_relative_target
  expect_tidy_error(
    p %>% add_rule_targets(0.3, "a", ft_data),
    "numeric"
  )
  expect_tidy_error(
    p %>% add_rule_targets(0.3, c(cr = NA_real_, vu = 0.2), ft_data),
    "missing"
  )
  expect_tidy_error(
    p %>% add_rule_targets(0.3, c(cr = 0.1, vu = 1.2), ft_data),
    "between"
  )
  expect_tidy_error(
    p %>% add_rule_targets(0.3, c(), ft_data),
    "numeric"
  )
  expect_tidy_error(
    p %>% add_rule_targets(0.3, c(cr = 0.1, greg = 0.4), ft_data),
    "names"
  )
  ## data
  expect_tidy_error(
    p %>% add_rule_targets(0.3, wt_data, "a"),
    "data frame"
  )
  expect_tidy_error(
    p %>% add_rule_targets(0.3, wt_data, ft_data[-1, ]),
  ...
  expect_tidy_error(
    {
      d <- ft_data;
      d$feature[[1]] <- "greg"
      p %>% add_rule_targets(0.3, wt_data, d)
    },
    "feature names"
  )
  expect_tidy_error(
