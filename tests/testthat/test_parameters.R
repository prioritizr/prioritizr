...
test_that('integer_parameter', {
  ...
  expect_equal(x$name , 'test')
  expect_true(inherits(x$id, 'id'))
  expect_equal(x$range, as.integer(c(-.Machine$integer.max,
                                          .Machine$integer.max)))
  ...
  expect_error(integer_parameter('test', -5))
  expect_error(integer_parameter('test', 5))
  expect_error(x$set(NA_real_))
  expect_error(x$set(Inf))
  expect_error(x$set(-5))
...
test_that('numeric_parameter', {
  ...
  expect_equal(x$name , 'test')
  expect_true(inherits(x$id, 'id'))
  expect_equal(x$range, c(.Machine$double.xmin,
                              .Machine$double.xmax))
  expect_equal(x$class, 'numeric')
  ...
  expect_error(numeric_parameter('test', 3L))
  expect_error(x$set(NA_real_))
  expect_error(x$set(Inf))
  expect_error(x$set(3L))
})

test_that('truncated_numeric_parameter', {
  x <- truncated_numeric_parameter('test', 1)
  # methods
  x$show()
  x$print()
  expect_is(x$repr(), 'character')
  expect_equal(x$name , 'test')
  expect_true(inherits(x$id, 'id'))
  expect_equal(x$range, c(0, .Machine$double.xmax))
  expect_equal(x$class, 'numeric')
  expect_equal(x$get(), 1)
  expect_false(x$validate(NA_real_))
  expect_false(x$validate(Inf))
  expect_false(x$validate(3L))
  expect_false(x$validate(-1))
  expect_true(x$validate(3))
  x$set(4)
  expect_equal(x$get(), 4)
  x$reset()
  ...
  expect_true(inherits(x$render(), 'shiny.tag'))
  # errors
  expect_error(truncated_numeric_parameter('test', NA_real_))
  expect_error(truncated_numeric_parameter('test', Inf))
  expect_error(truncated_numeric_parameter('test', 3L))
  expect_error(truncated_numeric_parameter('test', -1))
  expect_error(x$set(NA_real_))
  expect_error(x$set(Inf))
  expect_error(x$set(3L))
  expect_error(x$set(-1))
})

test_that('truncated_integer_parameter', {
  x <- truncated_integer_parameter('test', 1L)
  # methods
  x$show()
  x$print()
  expect_is(x$repr(), 'character')
  expect_equal(x$name , 'test')
  expect_true(inherits(x$id, 'id'))
  expect_equal(x$range, c(0, .Machine$integer.max))
  expect_equal(x$class, 'integer')
  expect_equal(x$get(), 1L)
  expect_false(x$validate(NA_real_))
  expect_false(x$validate(Inf))
  expect_false(x$validate(3))
  ...
  expect_true(inherits(x$render(), 'shiny.tag'))
  # errors
  expect_error(truncated_integer_parameter('test', NA_real_))
  expect_error(truncated_integer_parameter('test', Inf))
  expect_error(truncated_integer_parameter('test', 3))
  expect_error(truncated_integer_parameter('test', -1L))
  expect_error(x$set(NA_real_))
  expect_error(x$set(Inf))
  expect_error(x$set(3))
...
test_that('binary_parameter', {
  ...
  expect_error(numeric_parameter('test', NA_real_))
  expect_error(numeric_parameter('test', Inf))
  expect_error(numeric_parameter('test', 0))
  expect_error(numeric_parameter('test', -1L))
  expect_error(x$set(NA_real_))
  expect_error(x$set(Inf))
...
test_that('proportion_parameter_array', {
  ...
  expect_true(x$validate(data.frame(
                            value=c(0.1, 0.1, 0.2),
                            row.names=letters[1:3])))
  x$set(data.frame(value=c(0.9, 0.8, 0.7),
                   row.names=letters[1:3]))
  expect_equal(rownames(x$get()), letters[1:3])
...
test_that('truncated_numeric_parameter_array', {
  x <- truncated_numeric_parameter_array('test', c(0, 0.1, 5), letters[1:3])
  # methods
  x$show()
  x$print()
  expect_is(x$repr(), 'character')
  ...
  expect_equal(x$class, 'numeric')
  expect_equal(rownames(x$get()), letters[1:3])
  expect_equal(x$get()[[1]], c(0, 0.1, 5))
  expect_is(x$render(), 'shiny.tag.list')
  expect_false(x$validate(data.frame(
                            value=c(Inf, 0.1, 0.2),
                            row.names=letters[1:3])))
  ...
  expect_equal(x$get()[[1]], c(0, 0.1, 5))
  # errors
  expect_error(truncated_numeric_parameter_array('test',
                            value=c(Inf, 0.1, 0.2),
                            row.names=letters[1:3]))
  expect_error(truncated_numeric_parameter_array('test',
                            value=c(NA_real_, 0.1, 0.2),
...
test_that('parameters', {
  ...
  p2 <- truncated_numeric_parameter_array('p2',c(0, 10, 6), letters[1:3])
  x <- parameters(p1, p2)
  # methods
  x$show()
  x$print()
