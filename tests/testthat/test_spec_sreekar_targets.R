test_that("defaults", {
  # create object
  x <- spec_sreekar_targets()
  # tests
  expect_equal(x$args$rare_area_threshold, 200)
  ...
  expect_equal(x$args$common_relative_target, 0.3)
...
test_that("raster features", {
  ...
  correct_targets <- fs * loglinear_interpolation(fs, 200, 1, 250000, 0.3)
  ## apply target caps
