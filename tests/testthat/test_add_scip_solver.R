test_that("binary decisions", {
  ...
  p1 <-
    ...
    add_scip_solver(time_limit = 5, verbose = FALSE)
  p2 <-
    problem(sim_pu_raster, sim_features) %>%
    add_min_set_objective() %>%
    add_relative_targets(0.1) %>%
...
test_that("variable bounds methods", {
  ...
  p <-
    ...
    add_relative_targets(0.1) %>%
    add_binary_decisions() %>%
    add_scip_solver(first_feasible = TRUE, verbose = TRUE)
  # modify problem
...
test_that("first_feasible", {
  ...
  p1 <-
    ...
    add_relative_targets(0.1) %>%
    add_binary_decisions() %>%
    add_scip_solver(first_feasible = TRUE, verbose = FALSE)
  # solve problem
  s1 <- solve(p1)
...
test_that("control", {
  ...
  p1 <-
    ...
    add_scip_solver(
      ...
      control = list(verbosity_level = 2)
  )
  # solve problems
  s1 <- solve(p1)
  # tests
