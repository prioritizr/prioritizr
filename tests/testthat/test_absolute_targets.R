test_that("numeric (1)", {
  ...
  p <-
    ...
    add_auto_targets(absolute_targets(5))
...
test_that("numeric (5)", {
  ...
  p <-
    problem(sim_pu_raster, sim_features) %>%
    add_auto_targets(absolute_targets(v))
  # calculate absolute targets
  targets <- p$targets$output(p)
  # run tests
  expect_inherits(targets, "tbl_df")
...
test_that("invalid inputs", {
  ...
  # run tests
  expect_tidy_error(
    absolute_targets(p, 1),
    "add_absolute_targets"
  )
  expect_tidy_error(
    p %>% add_auto_targets(absolute_targets("a")) %>% compile(),
    "numeric"
  )
  expect_tidy_error(
    p %>% add_auto_targets(absolute_targets(c(1, 1))) %>% compile(),
    "length"
  )
  expect_tidy_error(
    p %>% add_auto_targets(absolute_targets(NA_real_)) %>% compile(),
    "missing"
  )
})
