...
test_that("numeric", {
  ...
  r1 <- eval_feature_representation(p, s)
...
test_that("matrix (multiple zones)", {
  ...
  r2 <- tibble::tibble(
    statistic = c(rep(c("overall", "1", "2"), each = 2)),
...
test_that("data.frame (multiple zone)", {
  ...
  idx <- which(!is.na(pu$cost_1) | !is.na(pu$cost_2))
  r2 <- tibble::tibble(
    statistic = rep(c("overall", "1", "2"), each = 2),
    feature = rep(c("1", "2"), 3),
    total_amount = c(
      sum(pu$spp1_1, pu$spp1_2, na.rm  = TRUE),
      sum(pu$spp2_1, pu$spp2_2, na.rm  = TRUE),
...
test_that("Spatial (multiple zone)", {
  ...
  idx <- which(!is.na(pu$cost_1) | !is.na(pu$cost_2))
  r2 <- tibble::tibble(
    statistic = rep(c("overall", "z1", "z2"), each = 2),
    feature = rep(c("spp1", "spp2"), 3),
    total_amount = c(
      sum(pu$spp1_1, pu$spp1_2, na.rm  = TRUE),
      sum(pu$spp2_1, pu$spp2_2, na.rm  = TRUE),
...
test_that("Raster (single zone)", {
  ...
  r2 <- tibble::tibble(
    statistic = rep("overall", nlayers(sim_features)),
