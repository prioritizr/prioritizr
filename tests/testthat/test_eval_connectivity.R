...
test_that("single zone (zone matrix = 1)", {
  ...
  r1 <- eval_connectivity(p, pu[, "solution"], zm, cm)
  # calculate connectivity (matrix)
  r2 <- eval_connectivity(p, pu[, "solution"], zm, as.matrix(cm))
...
test_that("multiple zones (zone matrix = 1)", {
  ...
  # calculate connectivity (dgCMatrix)
  r1 <- eval_connectivity(p, pu[, c("sol_1", "sol_2")], zm, cm)
  # calculate connectivity (matrix)
  r2 <- eval_connectivity(p, pu[, c("sol_1", "sol_2")], zm, as.matrix(cm))
  # calculate connectivity (array)
  r3 <- eval_connectivity(
