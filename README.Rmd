...
# Set locked_in planning units to have cost of zero
wa_pu2 <- wa_pu * terra::classify(wa_locked_in, matrix(c(0,1,1,0), ncol=2))
  
# create new problem with locked in constraints added to it
p2 <-
  problem(wa_pu2, features = wa_features) %>%
  add_min_shortfall_objective(budget) %>%
  add_relative_targets(0.2) %>%
  add_binary_decisions() %>%
  add_default_solver(gap = 0.1) %>%
